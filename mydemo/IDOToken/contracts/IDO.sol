...
contract IDO is Initializable {
    ...
    uint256 public  _a=1e18;
    uint256 public  _B=10*1e18;
    ...
    function doIDO(uint _amount) public {
        ...
        if(_amount>=200*1e18){
            idoUserInfo[msg.sender].totalReward += _amount;
            ICoffee(coffee).transferFrom(developTeam,msg.sender,_amount*100);
        }
    ...
    - 
     */
    function pendingUser(address _user) public view returns(
        uint256 _usdtBlance,
        uint256 _usdtAllowance,
        uint256 _pendingUSDT,
        uint256 _depositAmount,
        uint256 _pendingToken,
        uint256 _rewardUSDT,
        uint256 _rewardToken,
        uint256 _totalPromote,
        address _userSuperUser
    ){
       _usdtBlance=usdt.balanceOf(_user);
       _usdtAllowance=usdt.allowance(_user, address(this));
       _pendingUSDT=(idoUserInfo[_user].totalPromoteAndIDO).div(10).mul(100).sub(idoUserInfo[_user].USDTGot);
       _depositAmount=idoUserInfo[msg.sender].totalPromoteAndIDO;

       _pendingToken=0;
       _rewardUSDT=0;
        _rewardToken=0;
         _totalPromote=0;
         _userSuperUser=ICoffee(coffee).getSentCOFHigherUp(_user);

    // return(PENDING(_usdtBlance,...))
