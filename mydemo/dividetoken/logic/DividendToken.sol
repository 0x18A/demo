...
/**
@dev 合约发行之后，当前没有手续费逻辑暂时不需要设置参数
- 设置swapPair,用来判断手续费发放逻辑
- 设置vault金库合约为manner,用来进行燃烧
 */
contract DividendToken is IERC20, ManagerUpgradeable {
    ...
    function burn(address to, uint256 amount) public onlyManagers {
        _burn(to, amount);
    }
    ...
    function _transferStandard(
        ...
        uint256 useFee = 0;
        //如果是出售的时间，需要分发手续费
        if (swapPair != address(0) && !whiteList[from]) {
            if (to == swapPair) {
                useFee = 15;

                //将金额转给金库,调用金库发放手续费
                _tOwned[vault] = _tOwned[vault].add(
                    amount.mul(useFee).div(100)
                );
                emit Transfer(from, vault, amount.mul(useFee).div(100));
                IVault(vault).takeSellFee(amount, from);
            }
        }

        if (useFee != 0) {
            _tOwned[to] = _tOwned[to].add(amount.mul(85).div(100));
            emit Transfer(from, to, amount.mul(85).div(100));
        } else {
    ...
    @dev 设置金库地址
     */
    function updateVault(address _vault) public onlyManagers {
        vault = _vault;
    }
    ...
    @dev 设置配对地址
     */
    function updateSwapPair(address _swapPair) public onlyManagers {
        swapPair = _swapPair;
    }
